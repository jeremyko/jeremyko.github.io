...
jep 은 python 과 흡사한 대화형 콘솔도 제공한다. `pip install jep` 로 설치하면 python이 설치된 Scripts 폴더(예: `C:\Python311\Scripts`)에 **`jep.bat`** 파일이 생성되는데, 직접 python 파일(위 예제같이 java도  사용 할수있는)을 실행 하거나 대화형 콘솔로 사용 할수있다.

![대화형console](/assets/img/jep_interactive.jpg){: width="900"}

python 파일을 직접 실행 해보자. java 를 호출하게 해본다.
