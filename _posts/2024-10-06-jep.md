...
![image](/assets/img/20241006_1.jpg){: width="200"}

최근 프로젝트에서 알아볼 시간도 없어서 그냥 이것으로 개발했는데, 하다보니 이런 식으로 개발 하는것은 전혀 바람직하지 않다는 시행 착오를 겪게 되었다. 
pandas 함수를 java 에서 tablesaw 이용해서 구현하고 결과가 python 과 동일한지 전부 검증 하는 방식으로 개발을 해야 했다. 
